...
사용자가 이미 채워 둔 칸은 두 번 지킵니다 — 프롬프트에 '이미 정해진 항목' 으로 넣고,
응답을 받은 뒤에도 `Object.assign(character, current)` 로 다시 덮습니다. 모델이 지시를 무시하고
다시 쓰는 경우가 있기 때문입니다.

`looksUsable()` 이 이름 + 살점 한 항목을 확인합니다. 모델이 시트 대신 "무엇을 도와드릴까요?"
같은 잡담을 돌려주면 502 와 함께 원문 앞부분을 `raw` 로 보냅니다 — 빈 칸이 조용히 덮이는 것보다
낫습니다.
